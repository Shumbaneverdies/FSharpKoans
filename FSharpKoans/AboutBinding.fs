...
*)
   ...
   *)
    ...
    let ``08 An identifier pattern will match anything`` () =
        let x = 45 // replace with an integer
        let y = "hey" // replace with a string
    ...
    let ``09 A wildcard pattern will match anything`` () =
        let _ = 4 // replace with an integer
        let _ = "hello moocow" // replace with a string
